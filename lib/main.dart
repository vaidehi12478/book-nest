...
decoration: BoxDecoration(
image: DecorationImage(
image: NetworkImage("https://cdn.pixabay.com/photo/2017/08/06/22/01/books-2596809_1280.jpg"),
fit: BoxFit.cover,
),
),
...
child:Image.network(
  bookIcons[widget.title] ?? "https://media-hosting.imagekit.io/23abcfbc01b54935/9788126930784_456x700.webp?Expires=1839241893&Key-Pair-Id=K2ZIVPTIP2VGHC&Signature=CWAiCwWw052dyouilOFUGhYy2I4w7NpEB9If~nZwNf6iu3R0R2cZkooLIwSD2gHUzgymrwqqticE4cXSkR6nHv3InNIXuO2BV2Ni9Wb9jJ2AH7ppI1aghDwi5kbN087sB-KnyP-IbiyAxZ0DGqdjcrR2EkKa~MX1tifI9aGiqiW8E1HpwJ2KYyF6DhZDSWBctL5EFo3RZTpS7kUIoCM7247KRDLAXPxISVdU6eY2QbuKR0dRyPtaw4AjCDSgI9zvzVvuu2GeWE43MlIyBUlmsdoBKbLQwNpAXM4idFCk0rP52orieAMId4MbF2JW6s~2sjbcP9Ajo491LU39vXAzjg__",
width: 60,
height: 80,
fit: BoxFit.cover,
),
...
//     : () => addToIssuedBooks(book),
  onPressed: (book.issuedTo == null || book.issuedTo.isEmpty)
      ? () => addToIssuedBooks(book)
      : null,
  child: Icon(
    (book.issuedTo == null || book.issuedTo.isEmpty) ? Icons.add : Icons.check,
    color: Colors.white,
  ),
  backgroundColor: (book.issuedTo == null || book.issuedTo.isEmpty)
      ? Colors.green
      : Colors.red,
...
//
//   Future<void> addToIssuedBooks(Book book) async {
//     final url = '$apiUrl/api/books/issue'; // Replace with your backend API URL
//     SharedPreferences prefs = await SharedPreferences.getInstance();
//     String? userId = prefs.getString('user_id');
//
//     try {
...
//       if (response.statusCode == 200) {
//         print('Book issued successfully');
//         _showSuccess(jsonData['message'], context);
//         // Handle success (show confirmation, update UI, etc.)
//       }else if (response.statusCode == 400){
//         print('book not available');
//         _showError(jsonData['error'], context);
//       }
//         else{
//         print('Failed to issue book');
//         _showError(jsonData['error'], context);
//         // Handle failure (error messages, etc.)
//       }
//     } catch (e) {
//       print('Error: $e');
...
Book({required this.id ,required this.title, required this.author, required this.description, required this.issuedTo});
